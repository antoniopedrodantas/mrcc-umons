...
def bhatta(l1, l2):
    l1 = np.array(l1)
    l2 = np.array(l2)
    num = np.sum(np.sqrt(np.multiply(l1, l2, dtype=np.float64)),
                 dtype=np.float64)
...
def intersection(l1, l2, method):
    return cv2.compareHist(np.float32(l1), np.float32(l2), method)
...
def distance_f(l1, l2, distanceName):
    ...
    if distanceName == "Euclidienne":
        ...
        if distanceName == "Correlation":
            ...
            distance = intersection(l1, l2, methode)
